...
/**
 * @title CuriaEscrow
 * @dev This contract implements an escrow mechanism with a deadline.
...
contract CuriaEscrow {
    ...
    modifier onlyParties() {
        require(
            ...
            "Only the parties can perform this action"
        );
        _;
    }

    /**
     * @dev Initializes the contract by setting the parties involved and the escrow deadline.
    ...
    function _arbiterWithdrawFunds(address recipient, uint256 amount) private {
    ...
    function settle(
        ...
        require(
            totalPayout <= totalDeposits,
            "Total payout must not exceed total deposits"
        );

        if (amountToDepositor > 0) {
            _arbiterWithdrawFunds(depositor, amountToDepositor);
        }

        if (amountToBeneficiary > 0) {
            _arbiterWithdrawFunds(beneficiary, amountToBeneficiary);
        }
